...
protocol PhotoLibraryServiceInputs {
    ...
    func freshFetch(_ currentCount: Int)
}
...
//        let scheduler = schedulerProvider.scheduler(with: .queue(.init(name: "com.poeticsyntax.Photo")))
        ...
        freshFetchIO.output
            .observeValues { [weak self] currentCount in
                ...
                self?.fetchImagesIO.input.send(value: -1) // TODO: this is super hacky
        }
...
//            resetCachedAssets()
    ...
    private let freshFetchIO = Signal<Int, NoError>.pipe()
    func freshFetch(_ currentCount: Int) {
        freshFetchIO.input.send(value: currentCount)
    }
